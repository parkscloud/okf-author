...
| D12 | Timestamp policy (`timestamp` field) | Write `timestamp` as an ISO 8601 datetime in **UTC** (trailing `Z`, real time of day). **Authoring / live-editing** uses the **actual current UTC time** read from the system clock (`date -u +%Y-%m-%dT%H:%M:%SZ`), refreshed on each meaningful edit; **conversion** uses the source file's **last-modified date** at midnight (`…T00:00:00Z`). Never emit a local-timezone offset. | Researched, not assumed: the vendored spec (§4.1) defines `timestamp` as the ISO 8601 datetime of *last meaningful change*, and every spec example uses UTC `Z` (two with a real time of day); Google's upstream reference bundles all carry real, to-the-second UTC times (`…+00:00`, verified live 2026-06-27). The skill previously said only "use the current time," so the model — having no clock — defaulted to `00:00:00` or a local offset; reading the real system clock fixes both. `00:00:00Z` is retained for conversion as honest date-level precision when the modification time of day is unknown. `validate.py` already accepts every UTC form (warns only on non-ISO), so no validator change. Decided 2026-06-27. |

## 3. Open decisions (planning queue)

_All planning decisions are resolved — D1–D12 in §2 (D1–D11 on 2026-06-20; D12 on 2026-06-27)._ The finalized
